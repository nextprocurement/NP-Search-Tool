...
class TMmodel(object):
    ...
    def get_tpc_labels(self):
        ...
        try:
            ...
            self._logger.error(
                f"An error occurred while trying to get labels for topics: {e}. Getting them one by one...")
            try:
                labels = [tl.get_label(desc) for desc in aux]
            except Exception as e:
                self._logger.error(
                    f"An error occurred while trying to get labels for topics: {e}. Returning empty labels...")
                return [""] * len(aux)
